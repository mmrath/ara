...
use tide::Context;
use crate::shared::state::TideContextExt;

pub async fn sign_up(mut cx: Context<()>) -> Result<Json<User>, HttpError> {
    let sign_up = cx.body_json()?;
    let context = cx.unauth_context();
    user::sign_up(&conn, &sign_up)
        .map(Json)
        .map_err(From::from)
}
