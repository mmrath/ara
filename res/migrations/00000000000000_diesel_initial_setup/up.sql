...
CREATE UNIQUE INDEX uk_app_user__email ON app_user (lower(email));
CREATE UNIQUE INDEX uk_app_user__username ON app_user (lower(username));
CREATE UNIQUE INDEX uk_app_user__phone_number ON app_user (lower(phone_number));
...
CREATE UNIQUE INDEX permission_uk_01 ON permission (lower(application), lower(authority));
...
(
    ...
    CONSTRAINT user_role_fk_02 FOREIGN KEY (user_id) REFERENCES app_user (id)
);
...
(
       ...
       CONSTRAINT user_group_user_fk_02 FOREIGN KEY (user_id) REFERENCES app_user (id)
);
